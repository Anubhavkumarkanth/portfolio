...
export const projects: Project[] = [
  {
    ...
    kind: 'Python simulation · full-stack',
    tagline: 'Measures what skipped, paused or reduced SIP contributions actually cost over the long run.',
    summary:
    ...
    tags: ['Python', 'FastAPI', 'PostgreSQL', 'SQLAlchemy', 'Monte Carlo', 'Pydantic', 'React', 'TypeScript'],
    highlights: [
      'Python engine that quantifies friction: compounding loss, compliance rate and a 0–100 discipline score',
      'Monte Carlo over 1,000 randomised return paths, summarised as P10 / P50 / P90',
      'PostgreSQL schema with transactional writes; history query tuned with EXPLAIN ANALYZE',
    ],
    links: {
  ...
  },
  {
    slug: 'dietbot',
    name: 'Dietbot',
    kind: 'Machine learning · data pipeline',
    tagline: 'A calorie-intake model built from raw public health data, and kept only because it beats the standard formula.',
    summary:
    ...
    tags: ['Python', 'Pandas', 'Scikit-learn', 'NumPy', 'PostgreSQL', 'Streamlit', 'pytest'],
    highlights: [
      'Training set built from raw CDC files: 3 NHANES tables joined and filtered to 4,624 adults',
      'Four model designs tested against baselines; the Random Forest beats the standard formula (MAE 633 vs 651 kcal)',
      'Inherited prototype rebuilt as a tested Python package with PostgreSQL persistence',
    ],
    links: {
  ...
  },
  {
    ...
    kind: 'SQL & data modelling',
    tagline: 'A normalised PostgreSQL order database with analytical SQL reports and all-or-nothing transactions. Every query is hand-written, with no ORM.',
    summary:
...
ORDER BY category, rank_in_category;`,
    ...
    tags: ['SQL', 'PostgreSQL', 'CTEs', 'Window functions', 'Data modelling', 'Transactions', 'Java'],
    highlights: [
      'Six reporting queries: CTEs, RANK() OVER (PARTITION BY …), correlated subqueries, HAVING',
      'Seven-table schema in 3NF, with constraints enforced in the database rather than the app',
      'All-or-nothing order writes with row locks; an index evaluated with EXPLAIN ANALYZE',
    ],
    links: {
