  ...
  return (
    <div className="rounded-xl border border-line bg-ink-950/60 p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div role="group" aria-label="Transaction outcome" className="inline-flex rounded-lg border border-line bg-white/[0.02] p-1">
          {MODES.map((m) => (
            <button
              key={m.id}
              type="button"
      ...
      <div className="mt-5">
        <OmsTransaction key={mode} mode={mode} animate />
      </div>
